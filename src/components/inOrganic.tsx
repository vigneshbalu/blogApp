...
const InOrganic = () => {
  ...
  const [activeBlog, setActiveBlog] = useState(inOrganicBlogs[0]);
    ...
    <div className="text-[#f9fdff] p-8 items-center justify-center mt-2">
      ...
      <div className="md:flex flex-row justify-between gap-8 mt-6">
        <div className="bg-[#2b2e41] w-[300px] h-[90vh] flex flex-col items-center">
           {inOrganicBlogs.map((blog, index)=> <div key={index}>
             <button onClick={()=> {setActiveBlog(inOrganicBlogs[index])}} className="border-2 rounded-lg my-2 border- px-7 py-2 border-white">{blog.datePosted}</button>
           </div>)}
        </div>
